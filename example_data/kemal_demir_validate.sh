...
# Validate the Kemal Demir resources that are already loaded in HAPI against
# the Shoulder on FHIR IG. Pulls everything for Patient PAT-LONG-002 via
...
PATIENT_IDENT="${PATIENT_IDENT:-PAT-LONG-002}"
...
echo "=========================================="
echo "  Kemal Demir Server-Side Validation"
echo "=========================================="
...
if [ -z "$patient_id" ]; then
    ...
    echo "  Did you run load-kemal-demir.sh yet?" >&2
    exit 1
fi
...
WORK_DIR="$(mktemp -d -t kemal_validate.XXXXXX)"
...
if [ "$total_errors" -gt 0 ]; then
    ...
    KEEP_DIR="$SCRIPT_DIR/.kemal_validate_last_failure"
    rm -rf "$KEEP_DIR"
